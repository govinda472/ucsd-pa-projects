 ...
 * PID: A16378764
 * Sources used: tutors
 * It creates a doubly linked list, with methods to modify it.
 *  
 * 
 */
...
/**
   ...
   /**
 * This class stores creates a Iterator for the linked List implemented above. 
 * it contains iterator functions that can be use to get the index element, add ect.
 * 
 * Instance variables: 
 * idx; - the index of the right node in the list
 * left - The left node in linked list
 * right- the right node in the linked list
 * forward- boolean used to determine wether next or previous has been called
 * canRemoveOrSet- used to determine wether the node can be modified
 * 
 */
   ...
   protected class MyListIterator implements ListIterator<E> {
    ...
    /**
     * This method Initialises the Iterator by setting the instance variables to the first position
     *  @param right node to set to head.next
     * @param left node to set to head
     * 
     */
    ...
    /**
     * This method determines wether there is another non-sential node after the current right node 
     *
     * @return true if there is another element, false otherise.
     */

    public boolean hasNext(){


        if((right.equals(tail))||(idx==size)){
            return false;
        }
        else{return true;}
    ...
    /**
     * This method shifts the left and right node up in the iterator by 1 along with the index
     * 
     * @return the data of the old right node
     */
 ...
 /**
     * This method determines if there is a non sential node before the left node in the iterator
     *
     * @return returns false if left.prev is head, otherwise it returns true
     */
...
/**
     * This method shifts down the left and right node in the iterator by 1 along with the index
     * 
     * @return the data of the old left node
     */
    ...
    /**
     * This method returns the next index of the iterator
     * 
     * @return idx which the index of the rightnode
     */
    ...
    /**
     * This method returns the previous index of the iterator
     * 
     * @return returns the index of the left node
     */
    ...
    /**
     * This method adds a element to the linked list inbetween the left and right node
     * then it sets the iterator's left node to the newly added node 
     * it keeps the iterators right node the same
     * 
     */
    ...
    /**
     * This method sets a element to the right or left node
     * it sets to the element to right node if forward is false
     * it sets to the element to left node if forward is true
     * 
     */
    ...
    public void set(E element){
        ...
        if(!forward){right.setElement(element);}
        else{left.setElement(element);}
    }

/**
     * This method removes a node to the linked list from either the left or right
     * it removes from the right node if Forward is false
     * it removes from the left node if Forward is true
     * it decrements the index and size by 1
     */
